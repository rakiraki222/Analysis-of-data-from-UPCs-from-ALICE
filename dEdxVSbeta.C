...
void dEdxVSbeta(const std::vector<std::string>& filepaths) {
    TFile *outfile = new TFile("dEdxVSBeta.root", "RECREATE");

    TH2F *dedx_beta = new TH2F("dedx_beta", "dE/dx vs beta for 0.5 < p < 0.7 [GeV/c]", 300, 0.4, 1.0, 300, 0.0, 225);
        ...
        while ((key = (TKey *)next())) {
            if (strcmp(key->GetClassName(), "TDirectoryFile") == 0) {
                ...
                for (int iEntry = 0; iEntry < treeReducedTrack->GetEntries(); ++iEntry) {
                    ...
                    if (findable > 100) {
                       if (calc_abs_momentum(pT,phiangle,eta) > 0.5 && calc_abs_momentum(pT,phiangle,eta) < 0.7) {
                          dedx_beta->Fill(beta, dEdx);
                          }
                       }
                }
            }
    ...
    outfile->Close();
    delete outfile;
    std::cout << "Analysis complete. Results saved to dEdxVSBeta.root" << std::endl;
}
